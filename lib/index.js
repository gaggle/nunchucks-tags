...
class Tag {
  ...
  register (name, fn, options) {
    ...
    options = merge({ends: undefined, placeholder: undefined}, options)

    if (options.placeholder) {
      this.placeholders[name] = options.placeholder
    }
  ...
  render (str, context) {
    const cache = []
    const escapeContent = (s) => '<!--' + placeholderSecret + (cache.push(s) - 1) + '-->'

    const env = this.env
    return new Promise((resolve, reject) => {
      for (const name of Object.keys(this.placeholders)) {
        const placeholder = this.placeholders[name]
        str = str.replace(placeholder, escapeContent)
      }

      env.renderString(str, context, (err, result) => {
        if (err) return reject(err)
        const resolved = result.replace(rPlaceholder, (str, opts) => cache[opts])
        resolve(resolved)
      })
    })
  }
