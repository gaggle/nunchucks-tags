  ...
  render (str, context) {
    ...
    const escapeContent = (s, match) => {
      const uuids = match.split('\n').map(l => {
        const lStart = numWhitespaces(l)
        const rest = l.slice(lStart)
        return `${l.slice(0, lStart)}<!--${placeholderSecret}${(cache.push(rest) - 1)}-->`
      })
    ...
    return new Promise((resolve, reject) => {
      str = applyPlaceholders(str, this.placeholders)
...
const numWhitespaces = function (s) {
  return Math.max(s.search(rWhitespaceCounter), 0)
}
